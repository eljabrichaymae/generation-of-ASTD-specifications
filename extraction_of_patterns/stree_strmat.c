 ...
 */
int stree_match(SUFFIX_TREE tree, int *T, int N,
                STREE_NODE *node_out, int *pos_out)
{
  return stree_walk(tree, stree_get_root(tree), 0, T, N, node_out, pos_out);
}

int stree_walk(SUFFIX_TREE tree, STREE_NODE node, int pos, int *T, int N,
               STREE_NODE *node_out, int *pos_out)
{
 ...
 * Returns:  nothing.
 */
void stree_get_label(SUFFIX_TREE tree, STREE_NODE node, int *buffer,
...
int stree_get_leaf(SUFFIX_TREE tree, STREE_NODE node, int leafnum,
                   int **string_out, int *pos_out, int *id_out)
{
  int i;
  STREE_INTLEAF intleaf;
  STREE_LEAF leaf;
 ...
 */
int int_stree_insert_string(SUFFIX_TREE tree, int *S, int *Sraw,
                            int M, int strid)
{
  int i, slot, newsize;
...
int int_stree_walk_to_leaf(SUFFIX_TREE tree, STREE_NODE node, int pos,
                           int *T, int N, STREE_NODE *node_out, int *pos_out)
{
  int len, edgelen;
  int *edgestr;
  STREE_NODE child;
 ...
 * Returns:  The structure or NULL.
 */
STREE_NODE int_stree_new_node(SUFFIX_TREE tree, int *edgestr,
                              int *rawedgestr, int edgelen)
{
  STREE_NODE node;
